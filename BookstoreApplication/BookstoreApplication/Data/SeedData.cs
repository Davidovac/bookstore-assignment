using BookstoreApplication.Models;
using Microsoft.AspNetCore.Identity;

namespace BookstoreApplication.Data
{
    public static class SeedData
    {
        public static async Task InitializeAsync(IServiceProvider serviceProvider)
        {
            var roleManager = serviceProvider.GetRequiredService<RoleManager<IdentityRole>>();
            var userManager = serviceProvider.GetRequiredService<UserManager<User>>();

            var editor1 = await userManager.FindByNameAsync("mirko");
            if (editor1 == null)
            {
                editor1 = new User
                {
                    UserName = "mirko",
                    Email = "mirko@example.com",
                    Name = "Mirko",
                    Surname = "Mirkovic",
                    EmailConfirmed = true
                };
                await userManager.CreateAsync(editor1, "Mirko123!");
            }

            if (!await userManager.IsInRoleAsync(editor1, "Editor"))
            {
                await userManager.AddToRoleAsync(editor1, "Editor");
            }

            var editor2 = await userManager.FindByNameAsync("marko");
            if (editor2 == null)
            {
                editor2 = new User
                {
                    UserName = "marko",
                    Email = "marko@example.com",
                    Name = "Marko",
                    Surname = "Markovic",
                    EmailConfirmed = true
                };
                await userManager.CreateAsync(editor2, "Marko123!");
            }

            if (!await userManager.IsInRoleAsync(editor2, "Editor"))
            {
                await userManager.AddToRoleAsync(editor2, "Editor");
            }

            var editor3 = await userManager.FindByNameAsync("petar");
            if (editor3 == null)
            {
                editor3 = new User
                {
                    UserName = "petar",
                    Email = "petar@example.com",
                    Name = "Petar",
                    Surname = "Petrovic",
                    EmailConfirmed = true
                };
                await userManager.CreateAsync(editor3, "Petar123!");
            }

            if (!await userManager.IsInRoleAsync(editor3, "Editor"))
            {
                await userManager.AddToRoleAsync(editor3, "Editor");
            }
        }
    }
}
